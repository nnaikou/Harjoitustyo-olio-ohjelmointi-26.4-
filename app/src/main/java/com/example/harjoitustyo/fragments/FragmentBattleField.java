...
public class FragmentBattleField extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        LinearLayout view = (LinearLayout) inflater.inflate(
                R.layout.fragment_battle_field, container, false);

        home = Home.getHomeInstance();
        trainingArea = TrainingArea.getTrainingAreaInstance();
        battleField = BattleField.getBattleFieldInstance();

        buttonMove = view.findViewById(R.id.btnMoveLutemonsFromBattleField);

        // lets fetch the lutemons from battle field
        lutemons = battleField.getLutemonsInArray();
        ...
        buttonMove.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view2) {
                ...
                rgGroup = view.findViewById(R.id.rgGroupBattleField);
                switch (rgGroup.getCheckedRadioButtonId()) {
                    case R.id.rbToTrainingfromBattleField:
                        for(Lutemon lute : lutemonsToTransfer) {
                            trainingArea.addLutemonWithItsId(lute, lute.getId());

                            // and then we need also to delete the lutemon from battle field
                            battleField.deleteLutemonById(lute.getId());
                        }
                        // lets inform the user of successful transfer
                        Toast.makeText(getContext(), "Siirto tehty", Toast.LENGTH_LONG).show();
                        getActivity().finish();
                        break;
                    case R.id.rbToHomefromBattleField:fromBattleField:fromHome:
                        for(Lutemon lute : lutemonsToTransfer) {
